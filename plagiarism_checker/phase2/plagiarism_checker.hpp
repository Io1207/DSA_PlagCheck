...
struct tokenized_submission_t : public submission_t
{
    ...
    bool has_been_flagged=false; // To ensure we don't flag the same submission multiple times
    bool is_new_submission; // We are not flagging pre-existing submissions that are given in the constructor
    int patch_small=0;
    int patch_long=0;
    bool flagAsPlag=false;
...
protected:
    ...
    std::vector<std::shared_ptr<submission_t>> tokenized_submissions;
