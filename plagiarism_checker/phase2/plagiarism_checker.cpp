    ...
    const bool &is_new_submission)
{
    ...
    // Part 1: Get the hashes for each index

    const int minLength = 15;
    std::vector<long long> hashes(n - minLength + 1);
    ...
    {
        hashes[i - minLength] = hash;
        hash = ((hash * base) + vec[i]) % mod;
        hash = (hash - ((vec[i - minLength] * baseExp) % mod) + mod) % mod;
    }

    // Add the last hash
    hashes[n - minLength] = hash;

    // Part 2: Get the set of all hashes
}
