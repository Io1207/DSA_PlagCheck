...
int longestApproximatePatternMatch(const std::vector<int>& vector1, const std::vector<int>& vector2, int& startIdx1, int& startIdx2, double tolerance = 0.1) {
    ...
    for (int i = 0; i < vector1.size(); ++i) {
        for (int j = 0; j < vector2.size(); ++j) {
            ...
            // Extend the window as long as similarity meets the criteria
            while (i + len < vector1.size() && j + len < vector2.size()) {
                int val1 = vector1[i + len];
                int val2 = vector2[j + len];
